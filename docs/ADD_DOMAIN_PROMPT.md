...
- Register adapters only via `DOMAIN_CATALOG` in `lib/ingestion/registry.py`
- Never put domain names in `lib/core`, `lib/reasoning`, `lib/graph` helpers, or `lib/ingestion/runner.py`
...
7. Do NOT edit lib/core, lib/reasoning, or lib/ingestion/runner.py
...
1. Add a DomainSpec to DOMAIN_CATALOG in lib/ingestion/registry.py with:
   - domain_id: <DOMAIN_ID>
...
Hard rules:
- New code lives under domain/<DOMAIN_ID>/
- Register only via DOMAIN_CATALOG in lib/ingestion/registry.py
- Never put domain names in lib/core, lib/reasoning, lib/graph helpers, or runner.py
- Domain fields go in CanonicalEntity.attributes only
