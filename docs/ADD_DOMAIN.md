...
> **Core design rule:** `lib/core`, `lib/reasoning`, `lib/graph`, and
> `lib/solver` (aside from the generic `StubSolver`) contain **zero
...
| Reasoning pipeline | `lib/reasoning/` | No |
| **Domain package** | `domain/<name>/` | **Yes — new package** |
| **Catalog entry** | `lib/ingestion/registry.py` | **Yes — add a `DomainSpec`** |
...
There is a **single** registry: `lib/ingestion/registry.py`. CLI
...
You do **not** edit `lib/ingestion/__main__.py` or hardcode adapters in
`lib/ingestion/tool.py` — those consume the registry.
...
from lib.core.db import create_neo4j_driver
from lib.graph.nodes import create_entity_node, create_dependency_edge
...
```python
# domain/aviation/solver.py
from lib.core.solver import AffectedSubgraph, LiveState, ResponseOption, SolverResult
...
`apps/api/app.py` calls `resolve_solver(settings)` at startup:
...
```
  lib/core/           ← domain-agnostic interfaces and settings
  lib/reasoning/      ← ReAct pipeline
  lib/graph/          ← Neo4j helpers
  lib/llm/            ← inference + pgvector RAG
  apps/api/query.py    ← POST /query route
  lib/ingestion/runner.py
  lib/ingestion/tool.py   ← schema built from registry
  lib/ingestion/__main__.py
```
...
| Calling the inference API from an adapter | Don't — LLM/vector calls go through `lib/llm/` |
