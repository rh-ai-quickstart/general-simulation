...
Adapters are resolved from ``lib.ingestion.registry`` based on
``Settings.enabled_domains``.

Usage in the reasoning pipeline:
    from lib.ingestion.tool import get_ingestion_tool_schema, call_ingestion_tool
...
from lib.ingestion.registry import get_adapter_class, list_adapter_ids
