...
``/lib/core``, ``/lib/reasoning``, or ``/apps/api`` changes when you replace
``StubSolver`` with a real implementation.  See ``lib/solver/`` for the stub
...
class Solver(Protocol):
    ...
    To add a domain-specific solver:
      ...
      2. Register it on that domain's ``DomainSpec.solver`` in
         ``lib/ingestion/registry.py``.  ``resolve_solver()`` picks it up
